// 在包含 thread_pool.h 之前定义 DEBUG_THREAD_POOL 以启用日志
...
#include "thread_pool.h" // 线程池库的头文件
...
int main() {
    ...
    // 如果有 thread_pool_get_thread_count(pool) 这样的函数会更理想。
    ...
    if (running_tasks) {
        ...
        // 使用新的 free_running_task_names 函数释放内存
    ...
    // (可选测试) 尝试在销毁后使用池应为错误
    // TPOOL_LOG("Main: 尝试在销毁后添加任务 (应优雅失败)...");
    // int *late_task_arg = malloc(sizeof(int)); *late_task_arg = 999;
    // thread_pool_add_task(pool, my_task_function, late_task_arg, "LateTask"); // pool 在此处无效
