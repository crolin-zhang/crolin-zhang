 ...
 */
#include "thread_pool.h" // 包含不透明类型 thread_pool_t 和公共函数声明。
#include <stdio.h>       // 用于 TPOOL_LOG/TPOOL_ERROR (通过 thread_pool.h) 和 perror。
...
typedef struct task_node_s {
    task_t task;             /**< 实际的任务数据 (函数、参数、名称)。task_t 在 thread_pool.h 中定义。 */
...
static void *worker_thread_function(void *arg) {
    ...
    thread_pool_t *pool = thread_args->pool; // pool 是 struct thread_pool_s*
...
// (thread_pool_create, thread_pool_add_task, thread_pool_destroy, 
//  thread_pool_get_running_task_names, free_running_task_names
//  的 Doxygen 注释在 thread_pool.h 中，因为它们是公共 API)
